from .__init__ import pygame, os, load, scale
...
class Map:
    def __init__(self, window):
        ...
        self.current_map = np.array([[-1 for i in range(self.rows)] for j in range(self.cols)])
        self.texture_path = "textures/"
        self.texture_table = [scale(load(self.texture_path + file), (self.texture_size, self.texture_size)) for file in os.listdir(self.texture_path)]
        self.texture_table.append(scale(load(os.path.join("textures", "0.png")), (self.texture_size, self.texture_size)))
    ...
    def show_palette(self):
        ...
        map_row_index = 0
        texture_index = 0
        for texture in self.texture_table:
            self.window.blit(texture, (x, y))
            standardized_x = (x // 32)
            standardized_y = (y // 32)
            self.palette_map[map_row_index].append((texture_index, (standardized_x, standardized_y)))
            x += self.texture_size
            texture_index += 1
            if (x >= GAME_WIDTH):
